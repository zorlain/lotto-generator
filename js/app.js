  ...
  const doGenerate = () => {
    ...
    if (warningEl) {
      if (state.sets.length < SETS_PER_GENERATE) {
        warningEl.textContent = `설정한 조건이 너무 까다로워서 ${SETS_PER_GENERATE}개 중 ${state.sets.length}개만 생성됐어요. 필터 조건을 조금 완화해보세요.`;
  ...
  const syncZone = () => {
    ...
    if (config.zoneSpread.zoneTargets.some((v) => v !== null)) {
      zoneHint.textContent =
        total > 6
          ? `지정한 개수의 합(${total}개)이 6개를 초과했습니다. 조합을 찾기 어려울 수 있어요.`
          : `지정한 개수의 합: ${total} / 6개`;
    } else {
      zoneHint.textContent = "";
    }
...
function renderSeoMeta(stats) {
  ...
  const title = `무료 로또 번호 생성기 - 로또 6/45 ${no}회 당첨결과 반영 통계 분석`;
  const description =
    `로또 6/45 ${no}회(${date}) 당첨번호까지 반영한 전 회차 데이터를 분석해 통계 기반으로 번호를 추천합니다. ` +
    `홀짝비율·합계구간·출현빈도(HOT/COLD)·AC값까지 직접 설정 가능. 회원가입 없이 무료로 바로 이용하세요.`;
